...
insert into city values ('6','Rotterdam','NLD','Zuid-Holland','140000');
...
insert into city values ('547','Dobric','BGR','Varna','100399');insert into city values ('552','Bujumbura','BDI','Bujumbura','300000');
...
insert into city values ('646','Santa Ana','SLV','Santa Ana','139389');insert into city values ('762','Bahir','Dar','ETH Amhara','96140');
...
select City,Name from stationdata;
...
select  length(City) from stationdata;
select max(length(City))from stationdata   ;

select City,length(City) from stationdata order by length(city) ;

select distinct City from stationdata where City like'A%' or City like'E%' or City like'I%' or City like'O%' or City like'U%' order by City asc;

select distinct City from  stationdata where City like'%A' or City like'%B';


select distinct City from stationdata where  City not like'O%' or City not like'U%' ;
...
insert into product values('1','Sugar','100');
...
insert into sale values (1,1,01,'2019-1-21',2,100),
(2,2,02,'2021-2-12',1,130),
(3,3,03,'2022-5-10',2,150);

select * from sale;

select * from product as pd left join sale as s 
on pd.product_id=s.seller_id;

select * from product as pd right join sale as s
on pd.product_id=s.seller_id;

select * from sale as s full outer join product as pd
on s.seller_id=pd.product_id ;

select sale.price,product.product_name from product
left  join sale
on sale.seller_id=product.product_id;

select sale.price,product.product_id from product 
full join sale
on sale.seller_id=product.product_id;

select product_name from product where unit_price > 200;

drop table data_;

create table data_ as select * from product  where product_id in (1,3) ;

select *from data_;

create table record as select product_name,product_id from product;
